...
async def playlist(client, message):
    ...
    if not queue:
        await message.reply_text("**Tidak ada streaming disini!**")
    temp = []
    for t in queue:
        temp.append(t)
...
async def settings(client, message):
    ...
    if stats:
        ...
        await message.reply("**Nyalain dulu lagunya heh.**")
...
async def hfmm(_, message):
    ...
    if len(message.command) != 2:
        await message.reply_text(
            "**saya hanya mengerti** `/musicplayer on` **dan** `/musicplayer off`"
        )
        return
    ...
    if status == "ON" or status == "on" or status == "On":
        ...
        if not message.chat.id in DISABLED_GROUPS:
            await lel.edit("**Music player aktif.**")
            return
        DISABLED_GROUPS.remove(message.chat.id)
        await lel.edit(
            f"✅ **Music player sudah aktif, gas.**\n\n💬 {message.chat.id}"
        )
        ...
        if message.chat.id in DISABLED_GROUPS:
            await lel.edit("**Music player di matikan.**")
            return
        DISABLED_GROUPS.append(message.chat.id)
        await lel.edit(
            f"✅ **Music player di matikan, bye.**\n\n💬 {message.chat.id}"
        )
    else:
        await message.reply_text(
            "**saya hanya mengerti** `/musicplayer on` **dan** `/musicplayer off`"
...
async def play(_, message: Message):
    ...
    if audio:
        ...
        try:
            ...
            await lel.edit(
                "😕 **Maaf, saya tidak menemukan request mu**\n• Coba cek lalu koreksi"
            )
            print(str(e))
            return
        dlurl=url
    ...
    if chat_id in callsmusic.pytgcalls.active_calls:
        ...
        try:
            ...
            message.reply("**voice chat tidak aktif, tidak dapat memutar lagu**")
            return
        await message.reply_photo(
            photo="final.png",
...
async def lol_cb(b, cb):
    ...
    try:
        ...
        await cb.message.edit("❌ lagu tidak di temukan")
        return
    ...
    try:    
        ...
        pass
    try:
        thumb_name = f"thumb-{title}armussic.jpg"
        thumb = requests.get(thumbnail, allow_redirects=True)
        open(thumb_name, "wb").write(thumb.content)
    except Exception as e:
        print(e)
